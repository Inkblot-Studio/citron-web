 ...
 * every subdomain — including this site — receives it. We never parse the
 * cookie ourselves; we forward it to the identity API for validation.
 *
...
export async function getSessionUser(): Promise<SessionUser | null> {
  ...
  try {
    const res = await fetch(`${identityApi.replace(/\/$/, '')}/api/session`, {
      ...
      // Session validity must not be cached across users.
      cache: 'no-store',
    });
